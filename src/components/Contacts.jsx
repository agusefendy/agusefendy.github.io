...
const Contact = () => {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const text = `<b>Contact web portfolio</b> \nNama: ${name} \nEmail: ${email} \nMessage: ${message}`;
    const options = {
      method: "POST",
      headers: {
    ...
    const send = await fetch(
      "https://api.telegram.org/bot8696234674:AAFyINgKsJ3n1FYhR-y7ErMMdmFK7wn6B1E/sendMessage",
      options,
    );
    if (send.status == 200) {
          ...
          <form name="contact-form" onSubmit={submit}>
            <div className="w-full lg:w-2/3 lg:mx-auto">
              ...
              <div className="w-full px-4">
                ...
                >
                  {loading ? "Loading..." : t("contact.button")}
                </button>
              </div>
              <p
                id="n-success"
                className="font-medium text-md text-slate-500 md:text-lg mt-3 text-center hidden"
              >
                {t("contact.success")}
              </p>
              <p
                id="n-fail"
                className="font-medium text-md text-red-500 md:text-lg mt-3 text-center hidden"
              >
                {t("contact.fail")}
              </p>
            </div>
